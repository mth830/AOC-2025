...
const computeFinalDistance = () => {
  let i = 0;
  let count = 0;

  const maxMergeCount = pointList.length - 1;
  while (count < maxMergeCount) {
    ...
    if (count === maxMergeCount) {
      const finalIndex1 = pointList[a];
      const finalIndex2 = pointList[b];
      const [x1,y1,z1] = finalIndex1;
      const [x2,y2,z2] = finalIndex2;
      return x1 * x2;
    }
    i++;
  }
...
console.log(computeFinalDistance());